...
- [x] **T1 — Re-point the 13 per-row cross-player name reads through `players`.** Rewrite the affected `JOIN
...
- [x] **T2 — World lobby, join flow, switcher, nav.** `GET /worlds` (joined + joinable lists), `POST
...
- [x] **T3 — End-to-end multi-world loop + regression.** Integration: from the lobby, join a 2nd world →
