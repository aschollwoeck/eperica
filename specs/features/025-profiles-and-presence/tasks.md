...
- [ ] **T1 — Presence rule + bio validation (`domain/presence.rs`; P3/P7).** `Presence` + `presence(last,
...
- [ ] **T2 — `users.bio` + profile read/edit (migration `0036`).** `set_bio`, `profile_of -> ProfileView`
...
- [ ] **T3 — Profile page (bio + presence) + edit + touch middleware.** Public profile shows the bio + a
