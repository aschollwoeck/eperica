...
- [ ] **T1 — Define `WorldRules` + `load_world_rules()`.** `infrastructure/src/world_rules.rs`: the struct
...
- [ ] **T2 — Registry on the bundle.** `WorldRegistry::new` takes one `Arc<WorldRules>` (drop the ~15 rule
  args); fields + `build_and_spawn` read `self.world_rules.<set>`; `main.rs` + the test harness pass the
  bundle. Suite green. (AC3/AC4)

- [ ] **T3 — `AppState` on the bundle + handler sweep.** Replace `AppState`'s individual rule fields with
