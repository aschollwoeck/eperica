...
- [ ] **T1 — Use case takes validated offsets.** `application::create_world` gains `artifact_offset_secs` +
...
- [ ] **T2 — Env defaults + form inputs.** `AppState` carries the env offsets; `CreateWorldForm` gains
...
- [ ] **T3 — Acceptance + regression.** Integration: a custom schedule sets the world's release dates; an
