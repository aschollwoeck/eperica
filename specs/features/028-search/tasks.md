...
- [ ] **T1 — Coordinate parser (`domain/search.rs`; P3).** `parse_coordinate` for `x|y` / `(x|y)` / `x,y` /
  `x y`. **Unit tests:** each accepted form, whitespace, negatives; junk rejected (AC3).

## Persistence & ports

- [ ] **T2 — Search queries (migration `0039`).** `AccountRepository::search_players` (username prefix,
...
- [ ] **T3 — Search use-case (`application/src/search.rs`).** `search(accounts, alliances, q) ->
...
- [ ] **T4 — Search page + nav box.** `GET /search?q=` (public — no auth); a search form in `base.html`.
