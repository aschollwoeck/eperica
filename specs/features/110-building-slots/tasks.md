...
- [ ] **T1 — Specs.** This spec + tasks; supersede `003`'s "fixed slot per kind" decision; fulfill
  `game-design.md §3.2` (slot model + multiplicity + demolition). (no code)
- [ ] **T2 — Domain.** `slot: u8` on `BuildingSlot`; `VILLAGE_BUILDING_SLOTS = 22` + reserved set
...
- [ ] **T3 — Infra.** Read `village_buildings.slot` into `BuildingSlot.slot`; founding inserts the template's
...
- [ ] **T4 — Application.** Slot-centric `order_build` (validate via `can_place`; reserved kinds derive their
  ...
  processing.
- [ ] **T5 — Web build.** Slot-centric build POST (`slot` is the primary field; `kind` only for new
...
- [ ] **T6 — Web plan + demolish.** Render the 22 centre slots **by slot** (fixed CSS positions, reserved
...
- [ ] **T7 — Migration check.** Existing villages render/function unchanged; a single-Warehouse village's
