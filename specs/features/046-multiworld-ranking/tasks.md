...
- [x] **T1 — Population + climber boards.** Re-base `population_board` and `climber_board` on `players p
  WHERE p.world_id = $world` (name via `users`, return/group `p.id`, quadrant on the player column). Home
  parity; suite green. (AC1)

- [x] **T2 — Conflict boards.** Re-point `conflict_board` + `alliance_conflict_board` through `players p ON
  p.id = <pid> AND p.world_id = $world JOIN users`, adding the missing world filter. (AC1)

- [x] **T3 — Alliance population board + stat pages.** Re-point `alliance_population_board`; `player_stats`
...
- [x] **T4 — Search.** `search_players` → players in `self.world_id` joined to `users`. (AC3)

## Public pages

- [x] **T5 — `WorldScope` extractor + public pages.** Add the player-less/login-less extractor; migrate
  `leaderboard`, `wonder`, `search_page`, `player_stats_page`, `alliance_stats_page` to it. (AC4)

## Acceptance

- [x] **T6 — Second-world boards + regression.** Integration: a player joined to a 2nd world selects it →
