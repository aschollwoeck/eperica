...
- [x] **T1 — Pure protection/lifecycle rules + balance (`domain/lifecycle.rs`, P3/P7).** `LifecycleRules`;
...
- [x] **T2 — Migration + account protection/activity (`infrastructure`).** Migration `0029_lifecycle.sql`
...
- [x] **T3 — Beginner's protection in combat + threshold end.** `CombatError::TargetProtected`;
...
- [x] **T4 — `LifecycleRepository` + abandonment sweep.** `latest_swept_period`; `sweep_abandoned(period,
...
- [x] **T5 — Web: scheduler tick + protection status + map greying.** Scheduler gains `lifecycle_rules` + a
...
- [x] **T6 — Technical/end-user docs + review.** rustdoc on new public items;
