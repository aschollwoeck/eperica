...
- [ ] **T1 — Artifact model + effects + Treasury (`domain/artifact.rs`, `building.rs`; P3/P7).**
...
- [ ] **T2 — Release-date schedule (`worlds` + `World` + config).** Migration: `worlds.artifact_release_at`.
...
- [ ] **T3 — Natar materialization + artifact persistence (`infrastructure`).** Migrations: `users.is_npc`,
...
- [ ] **T4 — Capture & theft in the battle path (`application` + `infrastructure`).** `resolve_attack_one`
...
- [ ] **T5 — Effect wiring across the read hooks.** Fold `ArtifactEffects` (village small + account
...
- [ ] **T6 — Web: release tick, map, holdings panel, exclusions.** Scheduler `process_due_artifact_release`
...
- [ ] **T7 — Technical/end-user docs + review.** rustdoc on new public items;
