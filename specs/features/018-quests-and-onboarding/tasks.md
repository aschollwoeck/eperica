...
- [ ] **T1 — Pure quest rules + balance (`domain/quest.rs`, P3/P7).** `QuestId`, `QuestCondition`,
...
- [ ] **T2 — Migration + `QuestRepository` (`infrastructure`).** Migration `0028_quests.sql`
...
- [ ] **T3 — `evaluate_quests` use-case + hook.** Gather completed + progress, complete the gated met
...
- [ ] **T4 — `/quests` web page.** Authenticated page: current quest (description + reward), completed
...
- [ ] **T5 — Technical/end-user docs.** rustdoc on new public items; `docs/architecture/0020-quests.md`;
  `docs/manual/` quests & onboarding guide; `CLAUDE.md` active slice → 018.

- [ ] **T6 — Review & accept.** Full gates + P11; `eperica-reviewer` on the slice diff; fix until
  **APPROVE**; PR.
